...
public class PublicAccessFilter extends AbstractAuthFilter {
	...
	protected boolean process(AuthContext context) {
		...
		if (Objects.nonNull(permDefinition) && StringPool.ONE.equals(permDefinition.getPubAccessFlag())) {
			if (log.isDebugEnabled()) {
				log.debug("Public access api [{} {}] {},Skip filter", context.getPermDefinition().getMethod(),
						context.getPermDefinition().getServiceName(), context.getPermDefinition().getPath());
			}
			context.getResponseInfo().setProblem(AuthProblem.PUB_ACCESS);
			return false;
		}
		return true;
	}
