...
public class InternalAccessFilter extends AbstractAuthFilter {
	...
	protected boolean process(AuthContext context) {
		...
		if (Objects.nonNull(permDefinition) && StringPool.ONE.equals(permDefinition.getInternalAccessFlag())) {
			if (log.isDebugEnabled()) {
				log.debug("Internal access api [{} {}] {},Access denied", context.getPermDefinition().getMethod(),
						context.getPermDefinition().getServiceName(), context.getPermDefinition().getPath());
			}
			context.getResponseInfo().setProblem(AuthProblem.INTERNAL_ACCESS_DENIED);
			return false;
		}
		return true;
	}
