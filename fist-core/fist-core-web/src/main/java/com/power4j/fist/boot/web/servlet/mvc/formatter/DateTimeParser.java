...
public class DateTimeParser {
	...
	static {
		...
		YEAR_MONTH_FORMAT_REGISTRY.add(Pair.of("^\\d{4}-\\d{2}$", DEFAULT_YEAR_MONTH_FORMATTER));
		YEAR_MONTH_FORMAT_REGISTRY.add(Pair.of("^\\d{4} {1}\\d{2}$", DateTimeFormatter.ofPattern("yyyy MM")));
		YEAR_MONTH_FORMAT_REGISTRY.add(Pair.of("^\\d{6}$", DateTimeFormatter.ofPattern("yyyyMM")));

		MONTH_DAY_FORMAT_REGISTRY.add(Pair.of("^\\d{2}-\\d{2}$", DEFAULT_MONTH_DAY_FORMATTER));
		MONTH_DAY_FORMAT_REGISTRY.add(Pair.of("^\\d{2} {1}\\d{2}$", DateTimeFormatter.ofPattern("MM dd")));
		MONTH_DAY_FORMAT_REGISTRY.add(Pair.of("^\\d{4}$", DateTimeFormatter.ofPattern("MMdd")));

		DATE_FORMAT_REGISTRY.add(Pair.of("^\\d{4}-\\d{2}-\\d{2}$", DEFAULT_DATE_FORMATTER));
		DATE_FORMAT_REGISTRY.add(Pair.of("^\\d{4} {1}\\d{2} {1}\\d{2}$", DateTimeFormatter.ofPattern("yyyy MM dd")));
		DATE_FORMAT_REGISTRY.add(Pair.of("^\\d{8}$", DateTimeFormatter.ofPattern("yyyyMMdd")));

		TIME_FORMAT_REGISTRY.add(Pair.of("^\\d{2}:\\d{2}:\\d{2}$", DEFAULT_TIME_FORMATTER));
		TIME_FORMAT_REGISTRY.add(Pair.of("^\\d{2} {1}\\d{2} {1}\\d{2}$", DateTimeFormatter.ofPattern("HH mm ss")));
		TIME_FORMAT_REGISTRY.add(Pair.of("^\\d{6}$", DateTimeFormatter.ofPattern("HHmmss")));

		DATE_TIME_FORMAT_REGISTRY
				.add(Pair.of("^\\d{4}-\\d{2}-\\d{2} {1}\\d{1,2}:\\d{1,2}:\\d{1,2}$", DEFAULT_DATETIME_FORMATTER));
		DATE_TIME_FORMAT_REGISTRY
				.add(Pair.of("^\\d{4}-\\d{2}-\\d{2}.*T.*\\d{1,2}:\\d{1,2}:\\d{1,2}.*..*$", UTC_DATETIME_FORMATTER));
		DATE_TIME_FORMAT_REGISTRY.add(Pair.of("^\\d{8} {1}\\d{6}$", DateTimeFormatter.ofPattern("yyyyMMdd HHmmss")));
		DATE_TIME_FORMAT_REGISTRY.add(Pair.of("^\\d{14}$", DateTimeFormatter.ofPattern("yyyyMMddHHmmss")));
